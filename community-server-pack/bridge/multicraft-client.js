...
function createMulticraftClient(config = {}) {
  ...
  return {
    ...
    async start() {
      await call("startServer", { id: String(serverId) });
    },

    async stop() {
      await call("stopServer", { id: String(serverId) });
    },

    async restart() {
      await call("restartServer", { id: String(serverId) });
    },

    async resources() {
      const usage = await call("getServerResources", { id: String(serverId) });
      let server = {};
      try {
        const serverData = await call("getServer", { id: String(serverId) });
